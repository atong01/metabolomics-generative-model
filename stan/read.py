...
def metfrag_with_scores(path, keep_zero_scores = True):
    ...
    if not keep_zero_scores:
        to_return = dict((k,v) for k, v in to_return.items() if v != 0)
    return to_return
...
def dict_of_set(d, s):
    ...
    return dict((k,v) for k, v in d.items() if k in s)

def get_matrix_form(include_soft_evidence = True):
    pathways, features, path_dict, reverse_path_dict, evidence, metfrag_evidence = get_all_sets()
    pathways = np.array(list(pathways))
    metabolites = np.array(list(features))
    rev_paths = {p:i for i,p in enumerate(pathways)}
    rev_mets = {m:i for i,m in enumerate(metabolites)}
    P = pathways.shape[0]
    M = metabolites.shape[0]

    mask = np.zeros((P,M))
    for p,ms in path_dict.items():
        for m in ms:
            mask[rev_paths[p],rev_mets[m]] = 1
    evidence_vec = np.zeros((M,))
    for e,v in evidence.items():
        evidence_vec[rev_mets[e]] = v
    if include_soft_evidence:
        for e,v in metfrag_evidence.items():
            evidence_vec[rev_mets[e]] = v
    else:
        evidence_vec = evidence_vec.astype(np.int32)
    return rev_paths, rev_mets, mask.astype(np.int32), evidence_vec

if __name__ == "__main__":
    data = '../data/HilNeg 0324 -- Data.csv'
    d = get_model('../data/model2.csv')
    #mets = get_metabolites(d)
    rev_paths, rev_mets, mask, evidence = get_matrix_form(include_soft_evidence = False)
    print (np.sum(mask))
